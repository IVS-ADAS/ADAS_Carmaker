...
waypoint = {}; % Cell array to store individual waypoints
...
for route_idx = 1:numel(output)
    ...
    for point_idx = 1:size(route_data, 1)
        ...
        % Add the current waypoint to the current profile
        current_profile = [current_profile; waypoint_data];
    ...
    % Store the original route data as a single waypoint
    waypoint{end+1} = struct('id', route_idx, 'waypoint', route_data, 'numWaypoint', size(route_data, 1));
end
...
%         text(waypoints(wp, 1), waypoints(wp, 2), num2str(waypoints(wp)), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right');
%     end
end
